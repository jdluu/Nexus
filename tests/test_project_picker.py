...
@pytest.mark.asyncio
async def test_project_picker_mount_and_populate(mock_tool: Tool, tmp_path: Path) -> None:
    app: App[Any] = App()
    ...
    with patch("nexus.container.get_container") as mock_get_container:
        ...
        container.state_manager.get_recents.return_value = [str(tmp_path / "proj_recent")]
    ...
    with patch("nexus.container.get_container") as mock_get_container:
        ...
        async with app.run_test() as pilot:
            ...
            with patch.object(app, 'suspend'), \
                 patch('nexus.screens.tool_selector.ToolSelector.execute_tool_command') as mock_execute:
